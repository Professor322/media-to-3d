...
class NerfModel(nn.Module):
    ...
    def __init__(self, input_size_ray=3, input_size_direction=3, net_width=256):
        ...
        self.model_first_part = nn.Sequential(nn.Linear(input_size_ray, net_width), nn.ReLU(inplace=True))
        for _ in range(3):
            self.model_first_part = nn.Sequential(self.model_first_part, 
                                       nn.Linear(net_width, net_width),
                                       nn.ReLU(inplace=True))
        
        
        self.model_second_part = nn.Sequential(nn.Linear(input_size_ray + net_width, net_width),
                                               nn.ReLU(inplace=True))
        for _ in range(2):
            self.model_second_part = nn.Sequential(self.model_second_part,
                                                   nn.Linear(net_width, net_width),
                                                   nn.ReLU(inplace=True))
        ...
        self.model_color = nn.Sequential(nn.ReLU(), nn.Linear(net_width // 2, 3), nn.Sigmoid())
